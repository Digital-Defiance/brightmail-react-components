...
const ComposeModalInner: FC<ComposeModalProps> = ({
  ...
  useEffect(() => {
    if (defaultPreference && encryptionScheme === MessageEncryptionScheme.NONE) {
  ...
  const handleCloseAttempt = useCallback(() => {
    if (shouldConfirmClose(textBody, allRecipients, subject, attachments.length)) {
      setShowConfirm(true);
    } else {
      onClose();
    }
...
const ComposeModal: FC = () => {
  ...
  const portalContent = (
    <ComposeModalInner
      ...
      onSetDefaultEncryption={onSetDefaultEncryption as ((scheme: MessageEncryptionScheme) => Promise<void>) | undefined}
